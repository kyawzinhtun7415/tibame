...
AS
	SELECT e.ename "Employee", d.salary_vudname "Department", e.sal "Salary", s.grade "Grade"
    FROM emp e JOIN dept d ON (e.deptno = d.deptno) JOIN salgrade s ON (e.sal BETWEEN s.losal AND s.hisal);


-- 8. 在EMP資料表中利用ename欄位建立一個non-unique index 命名為
-- idx_emp_ename。
CREATE INDEX idx_exp_ename ON emp(ename);
